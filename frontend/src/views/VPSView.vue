<template>
  <div class="space-y-6">
    ...
    <div class="bg-white shadow-sm rounded-lg">
      <div class="px-4 sm:px-6 py-4 border-b border-gray-200">
        <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
          <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-900">VPS Management</h1>
            <p class="mt-1 text-sm text-gray-500">Manage your VPS servers and track expiration dates</p>
          </div>
        ...
        <div class="flex items-center space-x-4">
          <div class="flex items-center">
            ...
            <span class="text-sm font-medium text-blue-900">
              Search Results: {{ filteredVPS.length }} VPS servers found
            </span>
          </div>
    ...
    <div class="bg-white shadow-lg rounded-lg overflow-hidden">
      <!-- Loading State -->
      <div v-if="vpsStore.loading" class="p-8 text-center">
      ...
      <!-- Error State -->
      <div v-else-if="vpsStore.error" class="p-8 text-center">
        <div class="text-red-600 mb-4">
          ...
          <h3 class="text-lg font-medium">Error loading VPS servers</h3>
          <p class="mt-1">{{ vpsStore.error }}</p>
          <p class="mt-2 text-sm text-gray-500">
            {{ vpsStore.vpsList.length > 0 ? 'Showing fallback data. Some features may be limited.' : 'Unable to load any VPS data.' }}
          </p>
        </div>
        <div class="flex flex-col space-y-4 items-center">
          <button 
            @click="vpsStore.fetchVPSList()"
      ...
      <!-- Content Area -->
      <div v-else-if="!vpsStore.loading && !vpsStore.error && filteredVPS.length > 0">
        <!-- Desktop Table -->
        <div class="hidden lg:block overflow-x-auto">
          <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr v-for="vps in filteredVPS" :key="vps.id" class="hover:bg-gray-50 transition-colors duration-150">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    ...
                    <div class="ml-3">
                      <div class="text-sm font-medium text-gray-900">{{ vps.provider || 'Unknown' }}</div>
                ...
                <td class="px-6 py-4 whitespace-nowrap status-column">
                  <div class="flex items-center space-x-2 status-indicators">
                    ...
                    <div class="relative group">
                      <span :class="getComprehensiveStatusClass(vps.status, vps.expiresAt, 7, true, false)" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium status-badge">
                        <svg :class="getStatusIconClass(vps.status, vps.expiresAt)" class="mr-1.5 h-2 w-2 status-icon" viewBox="0 0 8 8">
                ...
                <td class="px-6 py-4">
                  <div v-if="vps.domains && vps.domains.length > 0" class="space-y-1">
                    ...
                    <div v-if="vps.domains.length > 1" class="text-xs text-gray-500">
                      +{{ vps.domains.length - 1 }} addon domain{{ vps.domains.length - 1 > 1 ? 's' : '' }}
                    </div>
                  </div>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div v-if="vps.password" class="flex items-center space-x-2">
                    <span class="text-sm text-gray-900">••••••••</span>
                    <button
                      @click="copyToClipboard(vps.id)"
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                  <div class="space-y-1">
                    <div class="font-medium">{{ formatDate(vps.expiresAt) }}</div>
                    <div v-if="vps.expiresAt" class="text-xs text-gray-500">
                      {{ getExpirationMessage(vps.expiresAt, 7, true, false) }}
                    </div>
                  </div>
                </td>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  ...
                  <button 
                    v-if="authStore.canCreateVPS"
                    @click="confirmDelete(vps.id)" 
                    :disabled="vps.domains && vps.domains.length > 0"
                    :title="vps.domains && vps.domains.length > 0 ? 'Cannot delete VPS with associated domains' : 'Delete VPS server'"
                    :class="[
                      vps.domains && vps.domains.length > 0 
          ...
          <div class="space-y-4 p-4">
            ...
            >
              <div class="flex items-start justify-between">
                <div class="flex items-center space-x-3">
                  ...
                  <div class="flex-1 min-w-0">
                    <h3 class="text-sm font-medium text-gray-900 truncate">{{ vps.provider }}</h3>
                    <p class="text-sm text-gray-500 truncate">{{ vps.name }}</p>
                  </div>
                </div>
                <div class="flex items-center space-x-2">
                  <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full" :class="getComprehensiveStatusClass(vps.status, vps.expiresAt, 7, true, false)">
                    {{ getExpirationMessage(vps.expiresAt, 7, true, false) }}
                  </span>
                </div>
              </div>
              ...
              <div class="mt-3 space-y-2">
                   ...
                   <span class="text-gray-900 font-medium">{{ vps.sshPort || 'N/A' }}</span>
                 </div>
                <div class="flex justify-between text-sm">
                  <span class="text-gray-500">IP Address:</span>
                  <span class="text-gray-900 font-medium">{{ vps.ipAddress || 'N/A' }}</span>
                </div>
                <div class="flex justify-between text-sm">
                  <span class="text-gray-500">Domains:</span>
                  <span class="text-gray-900 font-medium">
                    <span v-if="vps.domains && vps.domains.length > 0">
                      {{ vps.domains.length }} domain(s)
                    </span>
                ...
                <div class="flex justify-between text-sm">
                  ...
                  <span class="text-gray-900">{{ vps.username || 'N/A' }}</span>
                </div>
                <div class="flex justify-between text-sm">
                ...
                <div class="flex justify-between text-sm">
                  ...
                  <span class="text-gray-900">{{ formatDate(vps.expiresAt) }}</span>
                </div>
      ...
      <!-- No Results -->
      <div v-else-if="filteredVPS.length === 0 && !vpsStore.loading" class="p-8 text-center">
        <div class="text-gray-500">
          <svg class="mx-auto h-12 w-12 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <h3 class="text-lg font-medium">No VPS servers found</h3>
          <p class="mt-1 text-sm">
            {{ hasActiveFilters ? 'Try adjusting your search or filters.' : 'No VPS servers have been added yet.' }}
          </p>
          <div class="mt-4">
            <button 
    ...
    <DomainManagementModal
      ...
      entity-type="vps"
      :entity-id="selectedVPSForDomain?.id || ''"
      :entity-name="selectedVPSForDomain?.name || ''"
      :current-domains="selectedVPSForDomain?.domains || []"
...
import AddEditVPSModal from '@/components/modals/AddEditVPSModal.vue'
...
const selectedVPSForDomain = ref<VPS | null>(null)
const search = ref('')
...
const uniqueProviders = computed(() => {
  return [...new Set(vpsStore.vpsList.map(v => v.provider).filter((p): p is string => p !== null))].sort()
})

// Search placeholder
const searchPlaceholder = computed(() => {
  return 'Search VPS names and providers...'
})

// Client-side filtered VPS
const filteredVPS = computed(() => {
  let vpsList = vpsStore.vpsList

  // Apply search filter
  if (search.value && search.value.trim()) {
    const searchTerm = search.value.toLowerCase().trim()
    vpsList = vpsList.filter(vps => 
      vps.name.toLowerCase().includes(searchTerm) ||
      (vps.provider && vps.provider.toLowerCase().includes(searchTerm))
    )
  }

  // Apply status filter
  if (statusFilter.value) {
    vpsList = vpsList.filter(vps => vps.status === statusFilter.value)
  }

  // Apply provider filter
  if (providerFilter.value) {
    vpsList = vpsList.filter(vps => vps.provider === providerFilter.value)
  }
...
const openAddEditModal = (vps: VPS | null = null) => {
...
const handleSave = async (vpsData: Partial<VPS>) => {
  try {
...
const confirmDelete = async (id: string) => {
  // Find the VPS to check for associated domains
  const vps = vpsStore.vpsList.find(v => v.id === id)
  
  if (!vps) {
    alert('VPS server not found')
    return
  }
  
  // Check if VPS has associated domains
  if (vps.domains && vps.domains.length > 0) {
    const domainNames = vps.domains.map(d => d.name).join(', ')
    alert(`Cannot delete VPS server "${vps.name}" because it has associated domains: ${domainNames}\n\nPlease remove or reassign these domains first.`)
    return
  }
  
  // Confirm deletion for VPS without domains
  if (window.confirm(`Are you sure you want to delete the VPS server "${vps.name}"? This cannot be undone.`)) {
    try {
      const result = await vpsStore.deleteVPS(id)
      
      // Check if the result indicates an error
      if (result && result.status === 'error') {
        // Show error message to user
        alert(`Failed to delete VPS: ${result.message}`)
        return
      }
...
const openDomainModal = (vps: VPS) => {
  selectedVPSForDomain.value = vps
  isDomainModalOpen.value = true
}

const closeDomainModal = () => {
  isDomainModalOpen.value = false
  selectedVPSForDomain.value = null
}

const handleDomainUpdate = async () => {
  // Refresh VPS list to get updated domain information
  await vpsStore.fetchVPSList()
}

const copyToClipboard = async (vpsId: string) => {
  try {
    // Get the actual password from the API
    const password = await vpsStore.getVpsPassword(vpsId)
